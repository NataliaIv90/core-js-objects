...
function sellTickets(queue) {
  ...
  while (canSell && currentCustomerIndex < queue.length) {
    canSell = false;
    if (queue[currentCustomerIndex] === 25) {
      bills[25] += 1;
      canSell = true;
    }

    if (queue[currentCustomerIndex] === 50) {
      if (bills[25] > 0) {
        bills[25] -= 1;
        bills[50] += 1;
        canSell = true;
    ...
    if (queue[currentCustomerIndex] === 100) {
